...
class CCSparkJob(object):
    ...
    def parse_arguments(self):
        ...
        arg_parser.add_argument("--output_file", help=self.output_descr)
        
        arg_parser.add_argument("--whitelist_partition_path", help=self.output_descr)
    ...
    def run_job(self, session):
        ...
        # input_data = input_data.repartition(self.args.num_input_partitions)
        input_data = session.sparkContext.textFile(self.args.input, self.args.num_input_partitions)
        ...
        #     .saveAsTable(self.args.output_file)
        # session.createDataFrame(output, schema=self.output_schema) \
        #     .coalesce(self.args.num_output_partitions) \
        #     .write \
        #     .option("compression", self.args.output_compression) \
        #     .mode("overwrite") \
        #     .text(self.args.output_file)
        
        session.createDataFrame(output, schema=self.output_schema) \
            .rdd.coalesce(self.args.num_output_partitions) \
            .map(lambda x : str(x[0]) ) \
            .saveAsTextFile(self.args.output_file)
        # session.createDataFrame(output, schema=self.output_schema) \
        #     .rdd \
        #     .map(lambda x : str(x[0]) ) \
        #     .saveAsTextFile(self.args.output_file)
        ...
        #     .coalesce(self.args.num_output_partitions) \
        #     .write \
        #     .mode("overwrite").csv("s3://cc-test-abc/test-output/2022.csv")
    ...
    def load_whitelist_urls(self, uri):
        """Load whitelist partitions from S3 into a Spark DataFrame"""
        self.get_logger().info('Loading whitelist partitions from S3')
        
        partition_name = uri.split('/')[-1].replace('.warc.gz','')

        partition_path = self.args.whitelist_partition_path+'warc_norm_filename=' + partition_name
        
        # df  = session.read.parquet(partition_path)
        # url_enc_dict = {row['url']:row['content_charset'] for row in df.collect()}
        
        parDF2=pd.read_parquet(partition_path)
        url_enc_dict= dict(zip(parDF2['url'], parDF2['content_charset']))
    ...
    def process_warcs(self, _id, iterator):
        ...
        for uri in iterator:
            ...
            try:
                 # load whitelist partitions
                if self.args.whitelist_partition_path:
                    try:
                        uri_enc_dict = self.load_whitelist_urls(uri)
                    except Exception as e:
                        self.get_logger().error('Failed to load whitelist partitions: {}'.format(e))
                        self.warc_input_failed.add(1)
                        continue
                else:
                    uri_enc_dict = None
                ...
                for res in self.iterate_records(uri, archive_iterator,uri_enc_dict):
                    yield res
    ...
    def iterate_records(self, _warc_uri, archive_iterator,whitelist_dict=None):
        ...
        for record in archive_iterator:
            ...
            if whitelist_dict is not None:
                ...
                if record.rec_type == 'response' and url in whitelist_dict:
                    for res in self.process_record(record,whitelist_dict[url]):
                        yield res
